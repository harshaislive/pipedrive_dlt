...
def pipedrive_source(
    ...
    endpoints_resources = {}
    # Use direct pagination for core resources to ensure full historical sync
    for resource_name in ["pipelines", "stages", "deals", "users", "organizations", "persons", "products", "leads", "activities", "notes", "files"]:
        endpoints_resources[resource_name] = dlt.resource(
            get_pages,
            name=resource_name,
            primary_key="id",
            write_disposition="merge",
        )(resource_name, pipedrive_api_key)

    # Use recents only for frequently updated, less critical resources
    for entity, resource_name in RECENTS_ENTITIES.items():
        if resource_name not in endpoints_resources:
            endpoints_resources[resource_name] = dlt.resource(
...
def _get_deals_participants(
    ...
    total_deals_on_page = len(deals_page)
    for i, row in enumerate(deals_page):
        # Add verbose logging to see progress for each deal
        print(f"  -> Processing participants for deal {i+1}/{total_deals_on_page} (ID: {row['id']})")
